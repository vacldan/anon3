#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
SKRYI Deanonymization Folder Watcher Service
=============================================
Automaticky monitoruje slozku DEANON_IN a zpracovava dokumenty.

Pouziti:
1. Vloz anonymizovany dokument (*_anon.docx) a mapu (*_map.json) do slozky DEANON_IN
2. Pockej par sekund
3. Deanonymizovany dokument najdes ve slozce DEANON_OUT

Event-based monitoring (ne polling) - reaguje okamzite na nove soubory.
"""

import os
import sys
import time
import json
import shutil
import logging
import subprocess
from pathlib import Path
from datetime import datetime
from watchdog.observers import Observer
from watchdog.events import FileSystemEventHandler

# =============================================================================
# KONFIGURACE
# =============================================================================

# Zakladni slozka (Dokumenty/SKRYI)
def get_skryi_base_folder():
    """Vrati zakladni SKRYI slozku v Dokumentech uzivatele."""
    documents = Path.home() / "Documents"
    if not documents.exists():
        documents = Path.home() / "Dokumenty"  # Czech Windows
    if not documents.exists():
        documents = Path.home()
    return documents / "SKRYI"

BASE_FOLDER = get_skryi_base_folder()
DEANON_FOLDER = BASE_FOLDER / "02_DEANONYMIZACE"
IN_FOLDER = DEANON_FOLDER / "IN"
OUT_FOLDER = DEANON_FOLDER / "OUT"
ERROR_FOLDER = BASE_FOLDER / "ERROR"
LOGS_FOLDER = BASE_FOLDER / "LOGS"

# Podporovane formaty
SUPPORTED_EXTENSIONS = {'.docx'}

# Delay pred zpracovanim (cekame az se soubor dokonci kopirovat)
PROCESS_DELAY_SECONDS = 3

# =============================================================================
# LOGGING
# =============================================================================

def setup_logging():
    """Nastavi logging do souboru a konzole."""
    LOGS_FOLDER.mkdir(parents=True, exist_ok=True)

    log_file = LOGS_FOLDER / f"deanon_{datetime.now().strftime('%Y%m%d')}.log"

    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s [%(levelname)s] %(message)s',
        handlers=[
            logging.FileHandler(log_file, encoding='utf-8'),
            logging.StreamHandler(sys.stdout)
        ]
    )
    return logging.getLogger('SKRYI_DEANON')

# =============================================================================
# ZPRACOVANI DOKUMENTU
# =============================================================================

def get_app_install_path():
    """Najde instalacni slozku SKRYI Document Suite."""
    possible_paths = [
        # Standardni instalace x64
        Path(os.environ.get('PROGRAMFILES', 'C:\\Program Files')) / 'SKRYI Document Suite' / 'resources' / 'app.asar.unpacked',
        # Instalace x86
        Path(os.environ.get('PROGRAMFILES(X86)', 'C:\\Program Files (x86)')) / 'SKRYI Document Suite' / 'resources' / 'app.asar.unpacked',
        # LocalAppData (portable/user install)
        Path(os.environ.get('LOCALAPPDATA', '')) / 'SKRYI Document Suite' / 'resources' / 'app.asar.unpacked',
    ]

    for p in possible_paths:
        if p.exists():
            return p
    return None

def get_script_path(script_name):
    """Najde cestu ke skriptu (preferuje .exe pred .py)."""
    exe_name = script_name.replace('.py', '.exe')

    # Zkusime najit v ruznych lokacich
    possible_paths = []

    # 1. V instalacni slozce aplikace (PRIORITA pro nainstalovanou verzi)
    install_path = get_app_install_path()
    if install_path:
        possible_paths.append(install_path / exe_name)
        possible_paths.append(install_path / script_name)

    # 2. Vedle tohoto skriptu
    possible_paths.append(Path(__file__).parent / exe_name)
    possible_paths.append(Path(__file__).parent / script_name)

    # 3. Vedle sys.executable (pro zkompilovanou verzi)
    possible_paths.append(Path(sys.executable).parent / exe_name)
    possible_paths.append(Path(sys.executable).parent / script_name)

    for p in possible_paths:
        if p.exists():
            return p

    return None

def find_matching_pair(docx_path, logger):
    """
    Najde odpovidajici JSON mapu k anonymizovanemu dokumentu.

    Hledane vzory:
    - dokument_anon.docx -> dokument_map.json
    - dokument.docx -> dokument_map.json

    Returns:
        Path k JSON mape nebo None
    """
    docx_path = Path(docx_path)
    folder = docx_path.parent
    stem = docx_path.stem

    # Pokud nazev konci na _anon, nahrad _map
    if stem.endswith('_anon'):
        base = stem[:-5]  # odstran _anon
        possible_maps = [
            folder / f"{base}_map.json",
            folder / f"{stem}_map.json",  # dokument_anon_map.json
        ]
    else:
        base = stem
        possible_maps = [
            folder / f"{base}_map.json",
        ]

    for map_path in possible_maps:
        if map_path.exists():
            logger.info(f"Nalezena mapa: {map_path.name}")
            return map_path

    return None

def process_document(file_path, logger):
    """
    Zpracuje jeden dokument - deanonymizace.

    Args:
        file_path: Cesta k anonymizovanemu souboru
        logger: Logger instance

    Returns:
        bool: True pokud uspech, False pokud chyba
    """
    file_path = Path(file_path)
    logger.info(f"Zpracovavam: {file_path.name}")

    try:
        # Kontrola pripony
        if file_path.suffix.lower() not in SUPPORTED_EXTENSIONS:
            logger.warning(f"Nepodporovany format: {file_path.suffix}")
            return False

        # Najdi odpovidajici JSON mapu
        map_path = find_matching_pair(file_path, logger)
        if not map_path:
            logger.warning(f"Cekam na JSON mapu pro: {file_path.name}")
            return None  # None = cekej dal, mozna jeste prijde mapa

        # Deanonymizace
        logger.info("Spoustim deanonymizaci...")
        deanon_script = get_script_path('deanonymizator_lokal.py')

        if not deanon_script or not deanon_script.exists():
            install_path = get_app_install_path()
            logger.error(f"deanonymizator_lokal nenalezen! Install path: {install_path}")
            return False

        logger.info(f"Pouzivam CLI: {deanon_script}")

        # Priprav vystupni cesty
        stem = file_path.stem
        if stem.endswith('_anon'):
            base = stem[:-5]
        else:
            base = stem

        # Vystup primo do OUT_FOLDER s timestampem
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = OUT_FOLDER / f"{base}_{timestamp}_deanon.docx"

        # Volani s korektnimi argumenty
        cli_args = [
            str(deanon_script),
            "--input", str(file_path),
            "--map", str(map_path),
            "--output", str(output_file)
        ]

        # Nastav prostredi pro neinteraktivni rezim
        env = os.environ.copy()
        env['NO_PAUSE'] = '1'
        env['PYTHONIOENCODING'] = 'utf-8'

        if deanon_script.suffix == '.exe':
            result = subprocess.run(
                cli_args,
                capture_output=True, text=True, timeout=600,
                env=env
            )
        else:
            result = subprocess.run(
                [sys.executable] + cli_args,
                capture_output=True, text=True, timeout=600,
                env=env
            )

        if result.returncode != 0:
            logger.error(f"Deanonymizace selhala: {result.stderr}")
            logger.error(f"STDOUT: {result.stdout}")
            return False

        # Kontrola vystupu
        if output_file.exists():
            logger.info(f"Vystup: {output_file.name}")
        else:
            logger.error(f"Vystupni soubor nebyl vytvoren: {output_file}")
            return False

        # Smaz originaly z IN
        if file_path.exists():
            file_path.unlink()
            logger.info(f"Original smazan: {file_path.name}")

        if map_path.exists():
            map_path.unlink()
            logger.info(f"Mapa smazana: {map_path.name}")

        # Smaz i TXT mapu, pokud existuje
        txt_map = map_path.with_suffix('.txt')
        if txt_map.exists():
            txt_map.unlink()
            logger.info(f"TXT mapa smazana: {txt_map.name}")

        logger.info(f"Uspesne zpracovano: {file_path.name} -> {output_file.name}")
        return True

    except subprocess.TimeoutExpired:
        logger.error(f"Timeout pri zpracovani: {file_path.name}")
        return False
    except Exception as e:
        logger.error(f"Chyba pri zpracovani {file_path.name}: {e}")
        return False

def move_to_error(file_path, logger, error_msg=""):
    """Presune soubor do ERROR slozky."""
    try:
        file_path = Path(file_path)
        if file_path.exists():
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            dest = ERROR_FOLDER / f"{file_path.stem}_{timestamp}{file_path.suffix}"
            shutil.move(str(file_path), str(dest))
            logger.warning(f"Presunuto do ERROR: {dest.name}")

            # Zapis error log
            error_log = ERROR_FOLDER / f"{file_path.stem}_{timestamp}_error.txt"
            with open(error_log, 'w', encoding='utf-8') as f:
                f.write(f"Soubor: {file_path.name}\n")
                f.write(f"Cas: {datetime.now().isoformat()}\n")
                f.write(f"Chyba: {error_msg}\n")
    except Exception as e:
        logger.error(f"Nelze presunout do ERROR: {e}")

# =============================================================================
# FILE SYSTEM WATCHER
# =============================================================================

class DeanonHandler(FileSystemEventHandler):
    """Handler pro udalosti file systemu."""

    def __init__(self, logger):
        self.logger = logger
        self.processing = set()  # Soubory, ktere se prave zpracovavaji
        self.pending = {}  # Soubory cekajici na zpracovani {path: timestamp}
        self.waiting_for_map = {}  # Dokumenty cekajici na mapu {path: first_seen_time}

    def on_created(self, event):
        """Vola se kdyz je vytvoren novy soubor."""
        if event.is_directory:
            return

        file_path = Path(event.src_path)

        # Ignoruj docasne soubory
        if file_path.name.startswith('~') or file_path.name.startswith('.'):
            return

        # Zajimaji nas .docx a .json soubory
        if file_path.suffix.lower() not in {'.docx', '.json', '.txt'}:
            return

        self.logger.info(f"Novy soubor detekovan: {file_path.name}")

        # Pridej do pending s casovym razitkem
        self.pending[str(file_path)] = time.time()

    def on_modified(self, event):
        """Vola se kdyz je soubor modifikovan (napr. stale se kopiruje)."""
        if event.is_directory:
            return

        file_path = str(event.src_path)
        if file_path in self.pending:
            # Aktualizuj timestamp - soubor se stale meni
            self.pending[file_path] = time.time()

    def process_pending(self):
        """Zpracuje soubory, ktere jsou pripraveny (uz se nekopiruj)."""
        now = time.time()
        to_process = []

        for file_path, timestamp in list(self.pending.items()):
            # Pockej PROCESS_DELAY_SECONDS od posledni modifikace
            if now - timestamp >= PROCESS_DELAY_SECONDS:
                if file_path not in self.processing:
                    # Zajimaji nas pouze .docx soubory (JSON mapy se zpracuji automaticky s DOCX)
                    if file_path.lower().endswith('.docx'):
                        to_process.append(file_path)
                    del self.pending[file_path]

        for file_path in to_process:
            self.processing.add(file_path)

            # Zpracuj soubor
            result = process_document(file_path, self.logger)

            if result is None:
                # Ceka se na mapu - pridej do waiting_for_map
                if file_path not in self.waiting_for_map:
                    self.waiting_for_map[file_path] = time.time()
                    self.logger.info(f"Cekam na mapu: {Path(file_path).name}")
            elif result is False:
                move_to_error(file_path, self.logger, "Zpracovani selhalo")
            # result is True - vse OK

            self.processing.discard(file_path)

        # Zkontroluj soubory cekajici na mapu
        for file_path, first_seen in list(self.waiting_for_map.items()):
            if not Path(file_path).exists():
                # Soubor uz neexistuje
                del self.waiting_for_map[file_path]
                continue

            # Zkus znovu zpracovat
            map_path = find_matching_pair(file_path, self.logger)
            if map_path:
                self.logger.info(f"Mapa nalezena, zpracovavam: {Path(file_path).name}")
                result = process_document(file_path, self.logger)
                if result is False:
                    move_to_error(file_path, self.logger, "Zpracovani selhalo")
                del self.waiting_for_map[file_path]
            elif now - first_seen > 300:  # 5 minut timeout
                self.logger.warning(f"Timeout cekani na mapu: {Path(file_path).name}")
                move_to_error(file_path, self.logger, "Mapa nebyla nalezena do 5 minut")
                del self.waiting_for_map[file_path]

# =============================================================================
# HLAVNI SMYCKA
# =============================================================================

def create_folders():
    """Vytvori potrebne slozky."""
    for folder in [BASE_FOLDER, DEANON_FOLDER, IN_FOLDER, OUT_FOLDER, ERROR_FOLDER, LOGS_FOLDER]:
        folder.mkdir(parents=True, exist_ok=True)

    # Vytvor info soubor v IN slozce
    info_file = IN_FOLDER / "POUZITI.txt"
    if not info_file.exists():
        with open(info_file, 'w', encoding='utf-8') as f:
            f.write("SKRYI Document Suite - 02_DEANONYMIZACE\n")
            f.write("=" * 50 + "\n\n")
            f.write("JAK POUZIVAT:\n")
            f.write("-" * 50 + "\n")
            f.write("1. Vloz anonymizovany dokument (*_anon.docx) do teto slozky\n")
            f.write("2. Vloz odpovidajici mapu (*_map.json) do teto slozky\n")
            f.write("3. Pockej par sekund\n")
            f.write("4. Deanonymizovany dokument najdes ve slozce OUT\n\n")
            f.write("DULEZITE:\n")
            f.write("-" * 50 + "\n")
            f.write("- Nazvy souboru musi odpovidat:\n")
            f.write("  smlouva_anon.docx + smlouva_map.json\n")
            f.write("- Pokud vlozis jen dokument bez mapy, system pocka 5 minut\n\n")
            f.write("(c) 2026 Nixminds s.r.o.\n")

def write_status(status, message=""):
    """Zapise status do souboru (pro tray ikonu)."""
    status_file = BASE_FOLDER / ".deanon_status"
    try:
        with open(status_file, 'w', encoding='utf-8') as f:
            json.dump({
                'status': status,
                'message': message,
                'timestamp': datetime.now().isoformat(),
                'in_folder': str(IN_FOLDER),
                'out_folder': str(OUT_FOLDER)
            }, f)
    except:
        pass

def main():
    """Hlavni funkce - spusti watcher."""
    print("=" * 60)
    print("SKRYI Document Suite - Deanonymization Folder Watcher")
    print("=" * 60)

    # Vytvor slozky
    create_folders()
    print(f"\nSlozky vytvoreny v: {BASE_FOLDER}")
    print(f"  02_DEANONYMIZACE/IN:  {IN_FOLDER}")
    print(f"  02_DEANONYMIZACE/OUT: {OUT_FOLDER}")
    print(f"  ERROR:                {ERROR_FOLDER}")
    print(f"  LOGS:                 {LOGS_FOLDER}")

    # Nastav logging
    logger = setup_logging()
    logger.info("SKRYI Deanon Watcher spusten")

    # Vytvor handler a observer
    handler = DeanonHandler(logger)
    observer = Observer()
    observer.schedule(handler, str(IN_FOLDER), recursive=False)

    # Spust observer
    observer.start()
    write_status('running', 'Monitoruji slozku DEANON_IN')

    print(f"\nMonitoruji slozku: {IN_FOLDER}")
    print("Vlozte anonymizovany dokument a mapu do slozky DEANON_IN.")
    print("Pro ukonceni stisknete Ctrl+C\n")

    try:
        while True:
            time.sleep(1)
            handler.process_pending()  # Zpracuj cekajici soubory
    except KeyboardInterrupt:
        print("\nUkoncuji...")
        write_status('stopped', 'Zastaveno uzivatelem')
    finally:
        observer.stop()
        observer.join()
        logger.info("SKRYI Deanon Watcher ukoncen")

if __name__ == '__main__':
    main()
