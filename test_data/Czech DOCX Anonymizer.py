...
- Důslednější použití JSON knihovny pro detekci jmen
- Vylepšená inference nominativu (oprava pro "Říha" atd.)
- Lepší mapování variant na stejnou osobu
...
def load_names_library(json_path: str = "cz_names.v1.json") -> tuple[Set[str], Set[str]]:
    """Načte křestní jména a příjmení z JSON knihovny"""
    try:
        ...
        # Příjmení
        if 'surnames_no_diac' in data:
            surnames.update(data['surnames_no_diac'])
...
CZECH_FIRST_NAMES, CZECH_SURNAMES = load_names_library()
...
def infer_first_name_nominative(observed: str, surname_observed: str = "") -> Optional[str]:
    ...
    # Mužská jména
    for suf in ['ovi','em','e','u','a']:  # Přidáno 'a' pro genitiv
        if obs.lower().endswith(suf) and len(obs) > len(suf)+1:
            cand = obs[:-len(suf)]
            if normalize_for_matching(cand) in CZECH_FIRST_NAMES:
                return cand
    
    # Pro případ "Michalu" -> "Michal"
    if obs.lower().endswith('u') and len(obs) > 2:
        cand = obs[:-1]
        if normalize_for_matching(cand) in CZECH_FIRST_NAMES:
            return cand
...
def infer_surname_nominative(observed: str) -> str:
    """Vylepšená inference nominativu příjmení"""
    ...
    # Příjmení končící na -ek (Mareček, Novák -> Nováka)
    m = re.match(r'^(.*)ček(a|ovi|em|u|e|y|ou|ům|ách)?$', obs, flags=re.IGNORECASE)
    if m: return m.group(1) + 'ček'
    
    # Příjmení končící na -nek (Stanek -> Stanka)
    m2 = re.match(r'^(.*)nk(a|ovi|em|u|e|y|ou|ům|ách)?$', obs, flags=re.IGNORECASE)
    if m2: return m2.group(1) + 'nek'
    
    # Příjmení končící na -ek obecně
    if low.endswith(('ka','kovi','kem','ku','ke')) and len(obs) > 3:
        return re.sub(r'k(ovi|em|u|e|a)?$', 'ek', obs, flags=re.IGNORECASE)

    # Příjmení končící na -ec (Švec -> Švece)
    m3 = re.match(r'^(.*)c(e|i|em|ů|ích|ům|ech|emi|u|y)?$', obs, flags=re.IGNORECASE)
    if m3: return m3.group(1) + 'ec'

    # DŮLEŽITÉ: Příjmení končící na -a v nominativu (Říha, Procházka, Suchá)
    # Pokud "ovi" končí a má >3 znaky, mohlo by to být z "a" (Říhovi -> Říha)
    if low.endswith('ovi') and len(obs) > 3:
        potential_nom = obs[:-3] + 'a'
        # Zkontrolujeme, jestli to vypadá jako české příjmení
        norm = normalize_for_matching(potential_nom)
        if norm in CZECH_SURNAMES or norm.endswith(('a', 'ka', 'ova', 'ska', 'cka')):
            return potential_nom
        return obs[:-3] + 'a'  # Stejně to zkusíme

    # Další pády příjmení končících na -a
    for suf in ('ovi', 'ou', 'u', 'e', 'y', 'em'):
        if low.endswith(suf) and len(obs) > len(suf)+2:
            potential = obs[:-len(suf)] + 'a'
            norm = normalize_for_matching(potential)
            if norm in CZECH_SURNAMES:
                return potential

    # Obecné mužské příjmení
    for suf in ('em','e','u','y'):
...
def variants_for_first(first: str) -> set:
    ...
    if low.endswith('a'):
        ...
        if low.endswith('al'): V |= {f+'a', f+'u', f+'ovi', f+'em', f+'e'}  # Michal
...
def variants_for_surname(surname: str) -> set:
    ...
    if low.endswith('a') and len(s) >= 2:
        stem = s[:-1]
        out |= {s, stem+'y', stem+'ovi', stem+'ou', stem+'u', stem+'e', stem+'em'}  # Přidáno 'em'
        return out
...
CTX_ROLE   = re.compile(r'\b(pronaj[iímatel|n[aá]jemce|dlu[zž]n[iík|v[eě]řitel|objednatel|zhotovitel|zam[eě]stnanec|zam[eě]stnavatel|ručitel|spoludlu[zž]n[iík|jednatel|statut[aá]rn[ií]\s+z[aá]stupce|sv[eě]dek)\b', re.IGNORECASE)
CTX_LABEL  = re.compile(r'j[mn][eě]no\s*(,|a)?\s*př[ií]jmen[ií]', re.IGNORECASE)

def is_likely_firstname(token: str) -> bool:
    """Kontrola, zda token vypadá jako křestní jméno podle knihovny"""
...
def is_likely_surname(token: str) -> bool:
    """Kontrola, zda token vypadá jako příjmení podle knihovny"""
    if not token or not token[0].isupper(): return False
    norm = normalize_for_matching(token)
    # Zkontroluj knihovnu nebo typické koncovky českých příjmení
    return norm in CZECH_SURNAMES or any([
        norm.endswith(suf) for suf in ['ova', 'ek', 'ak', 'ik', 'sky', 'cky', 'ny', 'a']
    ])
...
class Anonymizer:
    ...
    def _extract_persons_to_index(self, text: str):
        ...
        for m in PAIR_RE.finditer(text_no_titles):
            ...
            # Kontrola kontextu pro vyloučení produktů
            ...
            # PŘÍSNÁ kontrola proti knihovně
            if is_likely_firstname(f_nom):
                # Pokud je křestní jméno v knihovně, přidáme osobu
                self._ensure_person_tag(f_nom, l_nom)
                continue

            # Pokud není v knihovně, zkontroluj kontext
            pre = text[max(0, s-160):s]
            post = text[e:e+160]
            has_ctx = CTX_PERSON.search(pre+post) or CTX_ROLE.search(pre+post) or CTX_LABEL.search(pre+post)
            
            # Pouze pokud má silný kontext a vypadá jako jméno
            if (has_ctx
                and f_tok[:1].isupper() and l_tok[:1].isupper()
                and not f_tok.lower() in ROLE_STOP and not l_tok.lower() in ROLE_STOP):
                # Přesto zkontroluj, jestli aspoň příjmení vypadá česky
                if is_likely_surname(l_nom):
                    self._ensure_person_tag(f_nom, l_nom)
    ...
    def _replace_remaining_people(self, text: str) -> str:
        ...
        for m in list(PAIR_RE.finditer(text_no_titles)):
            ...
            # Striktní kontrola proti knihovně
            if not is_likely_firstname(f_nom):
                # Pokud není v knihovně, musí mít velmi silný kontext
                pre = text[max(0, s-160):s]
                post = text[e:e+160]
                has_ctx = CTX_PERSON.search(pre+post) or CTX_ROLE.search(pre+post) or CTX_LABEL.search(pre+post)
                if not has_ctx:
...
def main():
    ...
    if args.names_json != "cz_names.v1.json":
        ...
        CZECH_FIRST_NAMES, CZECH_SURNAMES = load_names_library(args.names_json)
