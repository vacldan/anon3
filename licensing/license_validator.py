...
def validate_license(license_path="license.lic", verbose=True):
    ...
    if verbose:
        print("[LICENSE] Kontroluji licenci...")
    ...
    if verbose:
        print(f"[LICENSE] Načteno: {license_data.get('license_key', 'N/A')}")
    ...
    if current_hw_id != expected_hw_id:
        if verbose:
            print(f"[LICENSE] ✗ HW ID nesouhlasí")
            print(f"[LICENSE]   Očekáváno: {expected_hw_id}")
            print(f"[LICENSE]   Aktuální:  {current_hw_id}")
        return False, "Licence je vázána na jiné zařízení", None

    if verbose:
        print("[LICENSE] ✓ Hardware ID souhlasí")
    ...
    try:
        ...
        if verbose:
            print(f"[LICENSE] ✓ Platnost: {days_remaining} dní")
